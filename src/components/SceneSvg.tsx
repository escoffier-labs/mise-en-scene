...
function Architecture({ scene, byId, selectedId, review, spotlight, onSelect }: Props & { byId: Map<string, SceneBlock> }) {
  return <>{scene.edges.map((edge) => { const a = byId.get(edge.from)!; const b = byId.get(edge.to)!; const x1=a.x+a.w, y1=a.y+a.h/2, x2=b.x, y2=b.y+b.h/2; const dim=review&&!edge.factIds.length; return <g key={edge.id} data-edge-id={edge.id} role="button" tabIndex={0} className={`flow active ${selectedId===edge.id?"selected":""} ${dim?"ungrounded":""} ${walk(spotlight,edge.id,true)}`} onClick={()=>onSelect?.("edge",edge.id)} onKeyDown={onActivateKeyDown(()=>onSelect?.("edge",edge.id))}><path d={`M ${x1} ${y1} C ${x1+70} ${y1}, ${x2-70} ${y2}, ${x2} ${y2}`} markerEnd="url(#arrow)"/><text x={(x1+x2)/2} y={(y1+y2)/2-8} textAnchor="middle" className="flow-label">{edge.label}</text></g>})}{scene.blocks.map((block)=><Card key={block.id} block={block} selected={selectedId===block.id} dim={!!review&&!block.factIds.length} walk={walk(spotlight,block.id,false)} onSelect={onSelect}/>)}</>;
}
...
function Sequence({ scene, selectedId, review, spotlight, onSelect }: Props) {
  ...
  return <>{participants.map((block)=><g key={block.id}><line x1={cx(block)} y1={block.y+block.h} x2={cx(block)} y2={lifelineEnd} className="lifeline"/><Participant block={block} selected={selectedId===block.id} dim={!!review&&!block.factIds.length} walk={walk(spotlight,block.id,false)} onSelect={onSelect}/></g>)}{scene.edges.map((edge,index)=>{const a=byId.get(edge.from)!,b=byId.get(edge.to)!,y=Math.round(msgTop+index*step),x1=cx(a),x2=cx(b);return <g key={edge.id} data-edge-id={edge.id} role="button" tabIndex={0} className={`flow active ${selectedId===edge.id?"selected":""} ${review&&!edge.factIds.length?"ungrounded":""} ${walk(spotlight,edge.id,true)}`} onClick={()=>onSelect?.("edge",edge.id)} onKeyDown={onActivateKeyDown(()=>onSelect?.("edge",edge.id))}><text x={(x1+x2)/2} y={y-9} textAnchor="middle" className="flow-label">{edge.label}</text><path d={`M ${x1} ${y} L ${x2} ${y}`} markerEnd="url(#arrow)"/></g>})}</>;
}

function Card({ block, selected, dim, walk, onSelect }: { block: SceneBlock; selected: boolean; dim: boolean; walk: string; onSelect?: Props["onSelect"] }) { return <g data-block-id={block.id} role="button" tabIndex={0} aria-label={block.label} className={`scene-block active ${selected?"selected":""} ${dim?"ungrounded":""} ${walk}`} onClick={()=>onSelect?.("block",block.id)} onKeyDown={onActivateKeyDown(()=>onSelect?.("block",block.id))}><rect x={block.x} y={block.y} width={block.w} height={block.h} rx="10" className="card-rect"/><foreignObject x={block.x} y={block.y} width={block.w} height={block.h}><div className="card" {...({ xmlns: "http://www.w3.org/1999/xhtml" } as Record<string, string>)}><h3>{block.label}</h3><p>{block.detail}</p></div></foreignObject></g>; }
function Participant({ block, selected, dim, walk, onSelect }: { block: SceneBlock; selected: boolean; dim: boolean; walk: string; onSelect?: Props["onSelect"] }) { return <g data-block-id={block.id} role="button" tabIndex={0} aria-label={block.label} className={`scene-block active ${selected?"selected":""} ${dim?"ungrounded":""} ${walk}`} onClick={()=>onSelect?.("block",block.id)} onKeyDown={onActivateKeyDown(()=>onSelect?.("block",block.id))}><rect x={block.x} y={block.y} width={block.w} height={block.h} rx="9" className="card-rect"/><foreignObject x={block.x} y={block.y} width={block.w} height={block.h}><div className="participant" {...({ xmlns: "http://www.w3.org/1999/xhtml" } as Record<string, string>)}><span>{block.label}</span></div></foreignObject></g>; }
